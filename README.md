...
- 🗺️ OpenStreetMap integration for route visualization
...
- **Authentication**
  - Secure login & emergency contact management

- **Cloud Functions**
  - Orchestrates ML inference and routing updates
...
Traditional navigation minimizes only Distance, SafeRoute minimizes: Path Cost = Distance + (Risk Score × Safety Factor)
...
- Load and process spatio-temporal crime datasets  
- Train the XGBoost classifier  
- Generate and store production-ready model artifacts inside the `models/` directory  
...
- Temporary outputs and intermediate files are ignored.  
- Refer to `.gitignore` for the complete list of excluded files.
